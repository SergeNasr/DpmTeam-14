...
public class Albert_Algo {
	...
	public void take_data(boolean fromFrontSensor, boolean canGoForward){
		if(canGoForward){
			PathNode temp = currentPath.get(currentPath.size() - 1);
			currentPath.remove(currentPath.size() - 1);
			if(temp.isObstacleSeen()){
				int tilesAway = temp.getNodeTilesAway();
				for(int i = 0; i < tilesAway + 1; i++){
				}
			}
		} else {
